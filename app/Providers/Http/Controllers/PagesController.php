    ...
    public function orders(Request $request) {
        ...
        if(!empty($search) and !empty($sort)) {
            if($by === 'asc') {
                ...
                $orders = Orders::where('first_name', 'like', '%'.$search.'%')->orWhere('last_name', 'like', '%'.$search.'%')->orWhere('email', 'like', '%'.$search.'%')->orWhere('address', 'like', '%'.$search.'%')->orWhere('created_at', 'like', '%'.$search.'%')->orderBy($sort, 'DESC')->paginate(10);
                return view('pages.orders')->with('orders', $orders)->with('products', $products);
            }
            return view('pages.orders')->with('orders', $orders)->with('products', $products);
        }
